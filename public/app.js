...
// Contact form handler - moved to global scope
function sendContactForm() {
    console.log('sendContactForm called - function is accessible');
    
    // Test if function is in global scope
    if (typeof window.sendContactForm === 'undefined') {
        console.error('sendContactForm is not in global scope!');
    ...
    console.log('Form values:', { name, email, type, message });
    ...
    const subject = encodeURIComponent(`${getContactTypeLabel(type)} - ${name}`);
    const body = encodeURIComponent(
        `Name: ${name}\n` +
        `Email: ${email}\n` +
        `Inquiry Type: ${getContactTypeLabel(type)}\n\n` +
        `Message:\n${message}\n\n` +
        `---\n` +
        `Sent from Shortly Contact Form`
    );
...
function getTargetEmail(type) {
    const emails = {
        'sponsorship': 'sponsors@shortly.com',
        'partnership': 'business@shortly.com',
        'business': 'business@shortly.com',
        'support': 'support@shortly.com',
        'other': 'contact@shortly.com'
    };
    return emails[type] || 'contact@shortly.com';
}

// Ensure contact form functions are globally accessible
window.sendContactForm = sendContactForm;
window.getContactTypeLabel = getContactTypeLabel;
window.getTargetEmail = getTargetEmail;
